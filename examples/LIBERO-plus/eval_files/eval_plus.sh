...
Example:
  $(basename "$0") playground/Checkpoints/<run>/checkpoints/<checkpoint>.pt

Optional environment overrides:
  START_PORT, SERVER_ENV_NAME, EVAL_ENV_NAME, LIBERO_PLUS_HOME, LIBERO_CONFIG_PATH
USAGE
}
...
START_PORT="${START_PORT:-7694}"
HOST="${HOST:-127.0.0.1}"
SERVER_ENV_NAME="${SERVER_ENV_NAME:-starvla}"
EVAL_ENV_NAME="${EVAL_ENV_NAME:-liberoplus}"
LIBERO_PLUS_HOME="${LIBERO_PLUS_HOME:-${REPO_ROOT}/../LIBERO-plus}"
LIBERO_CONFIG_PATH="${LIBERO_CONFIG_PATH:-${LIBERO_PLUS_HOME}/libero}"
RESULTS_ROOT="${REPO_ROOT}/results/libero-plus"
LOCAL_LOG_ROOT="${REPO_ROOT}/.eval4_logs/libero-plus"
...
if [[ ! -f "${LIBERO_PLUS_HOME}/libero/libero/benchmark/task_classification.json" ]]; then
  echo "LIBERO-plus benchmark metadata not found under: ${LIBERO_PLUS_HOME}" >&2
  exit 1
fi
SERVER_SITE_PACKAGES="$("${SERVER_PY}" - <<'PY'
import site
paths = site.getsitepackages()
print(paths[0] if paths else "")
PY
)"
if [[ -z "${SERVER_SITE_PACKAGES}" || ! -d "${SERVER_SITE_PACKAGES}" ]]; then
  echo "cannot find server env site-packages" >&2
  exit 1
fi

ensure_libero_plus_config() {
...
datasets: ${DATA_ROOT}/data/libero-plus
...
find_render_gpu() {
  ...
  while IFS= read -r idx; do
    ...
    if MUJOCO_GL=egl PYOPENGL_PLATFORM=egl MUJOCO_EGL_DEVICE_ID="${idx}" \
      ...
      "${EVAL_PY}" - <<'PY' >/dev/null 2>&1
import os, sys
torch_site = os.environ.get("STARVLA_TORCH_SITE")
if torch_site:
...
launch_suite() {
  ...
  cat > "${local_out_dir}/log_paths.txt" <<TXT
launcher.log -> ${local_out_dir}/launcher.log
server.log -> ${local_out_dir}/server.log
eval.log -> ${local_out_dir}/eval.log
TXT
  rm -f "${out_dir}/log_paths.txt"
  cp -f "${local_out_dir}/log_paths.txt" "${out_dir}/log_paths.txt"

  nohup env \
    REPO_ROOT="${REPO_ROOT}" \
    LIBERO_PLUS_HOME="${LIBERO_PLUS_HOME}" \
    LIBERO_CONFIG_PATH="${LIBERO_CONFIG_PATH}" \
    SERVER_PY="${SERVER_PY}" \
    EVAL_PY="${EVAL_PY}" \
    CKPT="${CKPT}" \
...
set -euo pipefail
cd "${REPO_ROOT}"
export PYTHONPATH="${LIBERO_PLUS_HOME}:${REPO_ROOT}:${PYTHONPATH:-}"
export LIBERO_HOME="${LIBERO_PLUS_HOME}"
export LIBERO_CONFIG_PATH
export MUJOCO_GL=egl
export PYOPENGL_PLATFORM=egl
export TORCH_FORCE_NO_WEIGHTS_ONLY_LOAD=1
export STARVLA_TORCH_SITE
...
s.settimeout(1)
try:
    s.connect((os.environ["HOST"], port))
except Exception:
    sys.exit(1)
else:
    s.close()
...
MUJOCO_EGL_DEVICE_ID="${RENDER_GPU}" "${EVAL_PY}" ./examples/LIBERO-plus/eval_files/eval_libero.py \
  ...
  --args.log-path "${OUT_DIR}" \
  >"${LOCAL_OUT_DIR}/eval.log" 2>&1
STATUS=$?
rm -f "${OUT_DIR}/eval.exitcode"
...
start_summary_monitor() {
  local monitor_log="${LOCAL_RUN_ROOT}/summary.log"
  nohup env \
    RUN_ROOT="${RUN_ROOT}" \
...
exec >"${LOCAL_RUN_ROOT}/summary.log" 2>&1

echo "$(date -Iseconds) summary monitor start run_root=${RUN_ROOT} pids=$*"
for pid in "$@"; do
...
def pct(success, total):
    return None if total == 0 else 100.0 * success / total

def rounded_pct(success, total):
    value = pct(success, total)
    return None if value is None else round(value, 1)

aggregate = {category: {"success_count": 0, "total_count": 0} for category, _ in category_columns}
missing_suites = []

for suite in suites:
    json_path = run_root / suite / f"{suite}.json"
    if not json_path.exists():
        missing_suites.append(suite)
        continue
    data = json.loads(json_path.read_text(encoding="utf-8"))
    for category, values in data.items():
        bucket = aggregate.setdefault(category, {"success_count": 0, "total_count": 0})
        bucket["success_count"] += int(values.get("success_count", 0))
        bucket["total_count"] += int(values.get("total_count", 0))

summary = {
    "schema": "libero_plus_7d_summary_v1",
...
for category, column in category_columns:
    values = aggregate.get(category, {"success_count": 0, "total_count": 0})
    success = int(values["success_count"])
    count = int(values["total_count"])
    total_success += success
    total_count += count
    summary[column] = rounded_pct(success, count)

summary["Total"] = rounded_pct(total_success, total_count)

summary_path = run_root / "libero_plus_summary.jsonl"
if summary_path.exists():
    summary_path.unlink()
summary_path.write_text(json.dumps(summary, ensure_ascii=False) + "\n", encoding="utf-8")

print("LIBERO-plus README-style summary jsonl:", summary_path)
print(json.dumps(summary, ensure_ascii=False))
if missing_suites:
    print("missing suite JSONs:", ", ".join(missing_suites))
PY
echo "$(date -Iseconds) summary monitor done"
    ' bash "$@" >/dev/null 2>&1 < /dev/null &

  local monitor_pid=$!
  rm -f "${RUN_ROOT}/summary.pid"
  echo "${monitor_pid}" > "${RUN_ROOT}/summary.pid"
  echo "summary_monitor_pid=${monitor_pid} log=${monitor_log}"
}
...
start_summary_monitor "${LAUNCH_PIDS[@]}"
